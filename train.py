...
# update this for new data source
SOURCE = '/content/drive/MyDrive/training_us_soybean_corn_belt_new'
JOB_FOLDER = 'busan'  # update this for new models
JOB_DIR = JOB_FOLDER + '/trainer'
MODEL_DIR = '/content/drive/MyDrive/busan_model_new'
...
strat = tf.distribute.MirroredStrategy(devices=['/gpu:1'])  # '/gpu:0',
with strat.scope():
    from metrics import dice_coef, jacard_coef
    ...
    def log_confusion_matrix(epoch, logs):
        ...
        cm = sklearn.metrics.confusion_matrix(
            test_labels.reshape(-1, 1), test_pred.reshape(-1, 1))
    ...
    def tversky_loss(y_true, y_pred, alpha=0.5, beta=0.5):
        ...
        den = num + alpha * \
            tf.math.reduce_sum(p0*g1, (0, 1, 2)) + beta * \
            tf.math.reduce_sum(p1*g0, (0, 1, 2))
    ...
    tensorboard_cm_callback = tf.keras.callbacks.TensorBoard(
        log_dir=logdir_cm, histogram_freq=1)
    ...
    history = m.fit(
        ...
        callbacks=[tf.keras.callbacks.TensorBoard(
            LOGS_DIR), cp_callback,  cm_callback],
    )
